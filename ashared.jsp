    ...
    <center>
        <table style="width:80%">
            <%
                ...
                ResultSet r = MyConnection.getResultFromSqlQuery("select doc_id from share where uid='" + s + "'");
                int c = 0;
                while (r.next()) {
                    ++c;
                    int id = r.getInt(1);
                    ResultSet rs = MyConnection.getResultFromSqlQuery("select doc_id,filename,type, upload_time,upload_by from documents where Doc_id=" + id + "");
                    int count = 0;
                    while (rs.next()) {
                        ++count;
                        if (c == 1) {
                            out.print("<tr><th>File Name</th><th>File Type</th><th>Upload Time</th><th>Shared By</th><th>Action</th></tr><br/><br/>");
                        }
                        ...
                        out.println("<tr>"
                                ...
                                + "<td><a href='sdownload.jsp?Doc_id=" + rs.getInt(1) + "'> Download </a></td>"
                                + "</tr>" + "<br>");
